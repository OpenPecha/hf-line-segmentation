...
def extract_id_work_map(jsonl_dir):
    """Extracts id and work_id from JSONL files."""
    id_work_map = {}
...
def process_image(image_name, id_work_map):
    ...
    bdrc_work_id = ""
    for jsonl_id, work_id in id_work_map.items():
        if jsonl_id in id_value:
            bdrc_work_id = work_id
            break
...
def main(jsonl_dir, images_dir, output_jsonl):
    ...
    print(f"Output JSONL written to {output_jsonl}")
